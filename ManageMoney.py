def add_item(myTempList, item): # hàm thêm khoản chi tiêu
    myTempList.append(item)
def find_index_item(myTempList, item_name): # tìm khoản chi tiêu
    result = -1
    ...
    for i in range(length):
        if myTempList[i]['name'] == item_name:
            result = i
    return result 
def remove_item(myTempList, item_name): # xóa khoản chi tiêu
